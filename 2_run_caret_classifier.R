#------------------------------------------------------------------------------------
# Run the algorithms on the given data and compute the accuracy
#------------------------------------------------------------------------------------
...
run_caret_classifier <- function(x_train,y_train,x_test,y_test,classifier,path){
  ...
  ##----------------------  
    ...
    ##--------------- Save variable importance extracted from random forest
    fileConn = file(path)
  ...
  ##----------------------  Caret package classifiers
  }else{
    
    model = train(x_train, y_train, method = classifier, metric="Accuracy")
    
    ##--------------- Save variable importance extracted from lvq
    if(classifier=="lvq"){
      fileConn = file(path)
