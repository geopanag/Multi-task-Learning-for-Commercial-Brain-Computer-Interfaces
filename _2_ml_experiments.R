#------------------------------------------------------------------------------------
# The pipeline to run conventional algorithms in both datasets, with 5 and 10-fold cv
#    1. Load Datesets
#    2. Subset the data in cross validation folds based on subjects
#    3. Train and test each algorithm on that respective subsets and store the results
#------------------------------------------------------------------------------------
...
for(dataset in c("cmu","berkeley")){ #,
  ...
  for(cross_val_size in c(5,10)){
    ...
    #--- Matrices to keep the outcomes of the tests
    classifiers = c("LDA","sLDA","SVM","MLP","RBF_NN","LVQ","KNN","Ensemble","Tree","RF","XGB")
    ...
    for(f in seq_along(folds)){
      ...
      write.csv(train,paste0("Data/experiments/folds/train/",dataset,"_",cross_val_size,"_",f,".csv"),row.names=F)
      write.csv(test,paste0("Data/experiments/folds/test/",dataset,"_",cross_val_size,"_",f,".csv"),row.names=F)
      ...
      lvq_acc = run_caret_classifier(x_train,y_train,x_test,y_test,"lvq",
                                     paste0("Data/experiments/feature_selection/importance/",
                                            dataset,"_",cross_val_size,"_",f,"_lvq.csv"))
      if(!is.na(lvq_acc)){
      ...
      rf_acc = run_caret_classifier(x_train,y_train,x_test,y_test,"randomForest",
                                    paste0("Data/experiments/feature_selection/importance/",
                                           dataset,"_",cross_val_size,"_",f,"_rf.csv"))
    ...
    write.csv(results_acc,paste0("Data/experiments/conventional_results/",dataset,"_acc_",cross_val_size,".csv"),row.names=F)
